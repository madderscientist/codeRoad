...
def create_cqt_kernels(
    ...
    lengths = np.ceil(Q * fs / (freqs + gamma / alpha)) # 每个频点的窗长
    ...
    for k in range(0, int(n_bins)):
        ...
        # Centering the kernels 求的是窗的起始位置
        ...
        if norm:  # Normalizing the filter # Trying to normalize like librosa
            ...
            tempKernel[k, start : start + int(l)] = sig # 第k行是第k个频点的滤波器（时域）
...
def early_downsample_count(nyquist, filter_cutoff, hop_length, n_octaves):
    ...
    # 这里讨论的都是降2采样，返回降2采样的次数
    downsample_count1 = max(
        0, int(np.ceil(np.log2(0.85 * nyquist / filter_cutoff)) - 1) - 1    # 降采样后最高频率filter_cutoff不得高于新的奈奎斯特频率的85%（抗混叠）
    )
    # print("downsample_count1 = ", downsample_count1)
    num_twos = nextpow2(hop_length)
    # 由于这是第一次降采样，因此hop_length的时间跨度不能变（后续的降采样hop_length可以不随着降。但是nnAudio还是每个八度都除了2，以保证时域的均匀）
    downsample_count2 = max(0, num_twos - n_octaves + 1)    # 有b_octaves个八度，就要降采样b_octaves-1次，每次降采样hop_length也要变为一半，要防止hop_length<1。
