...
public:
    ...
    U8Drawer(uint16_t Width, uint16_t Height, uint8_t* parray = nullptr)
        : width(Width), height(Height), array(parray) {
        if (parray == nullptr) {
            array = new uint8_t[width * height << 1];  // 因为两位算一个像素
        }
    }
    U8Drawer(const U8Drawer& other) {
        width = other.width;
        height = other.height;
    ...
    */
    static uint16_t rgb2uint(uint8_t r, uint8_t g, uint8_t b) {
        r = ROUND(31 * float(r) / 255);
        g = ROUND(63 * float(g) / 255);
        b = ROUND(31 * float(b) / 255);
    ...
    static uint8_t* uint2rgb(uint16_t color) {
        U8Drawer::rgbBuffer[0] = ROUND(255 * float(color >> 11) / 31);
        U8Drawer::rgbBuffer[1] = ROUND(255 * float((color & 0b11111100000) >> 5) / 63);
        U8Drawer::rgbBuffer[2] = ROUND(255 * float(color & 0b11111) / 31);
        return U8Drawer::rgbBuffer;
    }
    /**
     * @brief  获取像素
     * @param  x x坐标
    ...
    */
    void gray() {
        ...
        for (uint32_t i = 0; i < size; i += 2) {
            uint8_t r = array[i] >> 3;
            uint8_t g = (array[i] & 0b111) << 3 | (array[i + 1] >> 5);
            uint8_t b = array[i + 1] & 0b11111;
            uint8_t gray = (r * 625 + g * 607 + b * 247 + 128) >> 8;
            r = ROUND(gray * 31.0 / 255);
            g = ROUND(gray * 63.0 / 255);
            b = ROUND(gray * 31.0 / 255);
            array[i] = r << 3 | g >> 3;
            array[i + 1] = (g << 5) | b;
        }
    }
};
uint8_t U8Drawer::rgbBuffer[3] = {0};
#endif