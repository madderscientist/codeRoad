...
class CQT1992(nn.Module):
    ...
    def __init__(
        ...
        cqt_kernels = fft(cqt_kernels)[:, : self.kernel_width // 2 + 1] # 求卷积核的频域，只取一半节省内存，且减少计算量
        ...
        # 注册成为module的parameter，这样在训练的时候可以自动求导，也可以导出
        if trainable_STFT:
    ...
    def forward(self, x, output_format=None, normalization_type="librosa"):
        ...
        # cqt_kernels_real只有前一半，fourier_real却是全部的，但是下面的乘法用的是matmul，为什么不报错？
        # 因为wcos的维度是(kernel_width/2, 1, kernel_width)，也就是保留了一半的频率
        # CQT
        CQT_real, CQT_imag = complex_mul(
            (self.cqt_kernels_real, self.cqt_kernels_imag), (fourier_real, fourier_imag)
        )

        CQT = torch.stack((CQT_real, -CQT_imag), -1)    # 共轭了

        if normalization_type == "librosa":
            CQT *= torch.sqrt(self.lenghts.view(-1, 1, 1)) / self.kernel_width  # length是每个卷积核的有效长度。不理解为什么长度在分子上
        elif normalization_type == "convolutional":
            pass
        elif normalization_type == "wrap":
            CQT *= 2 / self.kernel_width    # 这个是Parseval推导出来的。
        else:
            raise ValueError(
...
class CQT2010(nn.Module):
    ...
    def __init__(
        ...
        n_filters = min(bins_per_octave, n_bins)    # 一个八度内要几个滤波器
        self.n_octaves = int(np.ceil(float(n_bins) / bins_per_octave))  # 一共有几个八度
        # print("n_octaves = ", self.n_octaves)

        # Calculate the lowest frequency bin for the top octave kernel
        self.fmin_t = fmin * 2 ** (self.n_octaves - 1)  # 最高八度的最低频率（视fmin为最低八度的C）
        remainder = n_bins % bins_per_octave            # 最高八度有几个音
        ...
        self.fmin_t = fmax_t / 2 ** (   # 把最高音视为最高八度的B
            1 - 1 / bins_per_octave
        ...
        self.frequencies = freqs    # 所有84个音的频率

        lenghts = np.ceil(Q * sr / freqs)
        lenghts = torch.tensor(lenghts).float()
        self.register_buffer("lenghts", lenghts)    # 每个卷积核的有效长度
