...
for ain = 1:1:length(varargin)  % varargin是这样的: '键', 值, '键', 值, ... 是一个cell array（可以存不同数据类型），所以用大括号访问元素
...
%% define
octaveNr = ceil(log2(fmax/fmin));   % 八度的数目
fmin = (fmax/2^octaveNr) * 2^(1/bins); %set fmin to actual value 乘了2^(1/bins)其实是算了下一个音，因为计算范围是闭区间
xlen_init = length(x);

%% design lowpass filter 降2采样前的抗混叠滤波器 自己实现的时候可以用matlab计算的系数而不必实现butter
...
maxBlock = cqtKernel.fftLEN * 2^(octaveNr-1); %largest FFT Block (virtual)  最低频率的窗对应的FFT点数
suffixZeros = maxBlock; 
prefixZeros = maxBlock;
x = [zeros(prefixZeros,1); x; zeros(suffixZeros,1)]; %zeropadding
OVRLP = cqtKernel.fftLEN - cqtKernel.fftHOP;    % 重叠长度
